    ...
    <header className="bg-parchment sticky top-0 z-50 border-b border-border backdrop-blur-md">
      <nav className="flex justify-between items-center w-full px-6 lg:px-12 h-[72px] max-w-[1400px] mx-auto">
        ...
        <Link to="/" className="flex items-center shrink-0 flex-1">
          <span className="font-comfortaa text-[20px] md:text-[24px] font-black text-obsidian dark:text-white tracking-tight uppercase select-none">
            Asella
          </span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-4 shrink-0 flex-1 justify-end">
          ...
          {/* Theme Toggle */}
          <div className="w-10 h-10 rounded-full border border-border bg-white dark:bg-obsidian flex items-center justify-center text-obsidian/60 dark:text-white/70 hover:border-highland-gold hover:text-highland-gold transition-all shadow-sm">
            <ThemeToggle />
          </div>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-[72px] left-0 w-full bg-white dark:bg-obsidian border-b border-border shadow-lg animate-in slide-in-from-top-2 duration-200 py-4 px-6 flex flex-col gap-4">
          <a href="#hero" onClick={() => setMobileMenuOpen(false)} className="text-base font-bold text-obsidian dark:text-white hover:text-highland-gold uppercase tracking-widest py-2 border-b border-parchment-mid">Home</a>
          <a href="#products" onClick={() => setMobileMenuOpen(false)} className="text-base font-bold text-obsidian dark:text-white hover:text-highland-gold uppercase tracking-widest py-2 border-b border-parchment-mid">Products</a>
          <a href="#story" onClick={() => setMobileMenuOpen(false)} className="text-base font-bold text-obsidian dark:text-white hover:text-highland-gold uppercase tracking-widest py-2 border-b border-parchment-mid">About</a>
          <a href="#contact" onClick={() => setMobileMenuOpen(false)} className="text-base font-bold text-obsidian dark:text-white hover:text-highland-gold uppercase tracking-widest py-2 border-b border-parchment-mid">Contact</a>
          <Link to="/track" onClick={() => setMobileMenuOpen(false)} className="text-base font-bold text-obsidian dark:text-white hover:text-highland-gold uppercase tracking-widest py-2 border-b border-parchment-mid">Track Order</Link>

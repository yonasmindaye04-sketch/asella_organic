        ...
        BEGIN
            ...
            DECLARE item_cursor CURSOR FOR
                SELECT
                    ...
                    oi.item_name
                FROM order_items oi
                LEFT JOIN products p1 ON p1.id = oi.product_id AND p1.active = TRUE
                LEFT JOIN products p2 ON oi.product_id IS NULL
                      AND LOWER(oi.item_name) = LOWER(p2.name)
                      AND oi.package_size = p2.package_size AND p2.active = TRUE
